...
int main () {
	...
	for (int i = 0; i < N; i++) {
		if (transports[i].luggage_weight > average) {
			count++;
			cout << "Пассажир " << transports[i].surname << " " << transports[i].name << " " << transports[i].second_name << " имеет багаж, вес которого больше среднего веса \n";
		}
	}

	cout << "Всего пассажиров, вес багажа которых превосходит средний - " << count;
}
