...
int main () {
	...
	for (int i = 0; i < N; i++) {
		if (b[i]) {
			...
			for (int j = i+1; j < N; j++) {
				if (character_array_comparison(profiles[i].street_name, profiles[j].street_name) && profiles[j].age < 50) {
					b[j] = false;
			...
			if (male + female >= 1) {
				cout << "На улице " << profiles[i].street_name << " проживают " << male << " male и " << female << " female \n";
			}
			male = 0;
			female = 0;
		}
