...
int main () {
	...
	for (int i = 0; i < N; i++) {
		if (b[i]) {
			b[i] = false;
			if (profiles[i].gender == 'F' && profiles[i].age > 30) {
				female++;
			}

			for (int j = i+1; j < N; j++) {
				if (character_array_comparison(profiles[i].street_name, profiles[j].street_name) && profiles[i].house_number == profiles[j].house_number && profiles[j].age > 30) {
					b[j] = false;
					if (profiles[j].gender == 'F') {
						female++;
					}
				}
			}
			if (female >= 1) {
				cout << "На улице " << profiles[i].street_name << " в доме " << profiles[i].house_number << " проживают " << female << " female старше 30 лет \n";
			}
			female = 0;
		}
	}
